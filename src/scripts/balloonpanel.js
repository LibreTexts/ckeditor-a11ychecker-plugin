 ...
 */
 ...
 ( function() {
	...
	CKEDITOR.plugins.add( 'balloonpanel', {
		init: function() {
			if ( !stylesLoaded ) {
				CKEDITOR.document.appendStyleSheet('https://test.libretexts.org/alvin/ckeditor-a11ychecker/dist/balloonpanel/skins/moono-lisa/balloonpanel.css' );
				stylesLoaded = true;
			}
		}
	} );
	...
	CKEDITOR.ui.balloonPanel.prototype = {
		...
		templateDefinitions: {
			...
			triangleOuter: '<span style="display: none" class="cke_balloon_triangle cke_balloon_triangle_outer"></span>',

			triangleInner: '<span style="display: none" class="cke_balloon_triangle cke_balloon_triangle_inner">&#8203;</span>'
		},
		...
		move: function( top, left ) {
			this.rect.left = left;
			this.rect.top = top + 200;

			// ALVIN NOTE: This is where we change the positioning/styling of the balloon panel
			// Make it fixed to the right
			this.parts.panel.setStyles( {
				position: 'fixed',
				right: CKEDITOR.tools.cssLength( '20' ),
				top: CKEDITOR.tools.cssLength( '50vh' ),
...
export default addBalloonPanel;